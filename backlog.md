...
**Done criteria:** validador firma 20 normas sample con 0 errores graves. Release público v0.1 en GitHub. Tools 1-4 + `server_info` operativas. Cobertura: NCG vigentes + RAN + Compendio Seguros. **Audit trail completo desde la primera tool**.

### Infra y bootstrap

- [ ] **HdU-01 — Bootstrap del proyecto**
  Como dev, quiero `package.json`, `tsconfig.json` estricto, `biome.json`, `vitest.config.ts`, `tsup.config.ts` y `.gitignore` (excluyendo `data/`, `dist/`, `node_modules/`) listos, para poder ejecutar `pnpm install && pnpm typecheck && pnpm lint && pnpm test` sin errores en un repo vacío.
...
- [ ] **HdU-03 — Schema Drizzle (tablas core)**
  Como dev, quiero `src/db/schema.ts` con `norms`, `sections`, `articles`, `articles_history`, `norm_relations`, `validation_log` siguiendo DEVELOPER.md §4, para que el resto del sistema tenga una fuente de verdad tipada.
...
- [ ] **HdU-06 — Logging estructurado y audit trail (BLOQUEANTE para tools)**
  ...
  - Esta HdU **debe estar mergeada antes que cualquier tool** (HdU-10 a 14).

### Ingest

- [ ] **HdU-07 — Loader desde output del scraper**
  Como dev, quiero `src/ingest/loader.ts` que lea los `data/parsed/{normId}.json` producidos por el scraper y los inserte en SQLite respetando la convención de IDs (DEVELOPER.md §4), para poder poblar la DB sin re-scrapear.

- [ ] **HdU-08 — Validator de integridad post-ingest**
...
- [ ] **HdU-14 — Tool `search_articles` (FTS5 + BM25)**
...
- [ ] **HdU-15 — Bloqueo de normas no validadas en modo release**
...
- [ ] **HdU-16 — Errores explícitos por ID inexistente**
...
- [ ] **HdU-17 — Fixtures de 20 normas sample**
  Como dev, quiero `tests/fixtures/` con 20 normas representativas (mix NCG/RAN/Compendio) en formato JSON parsed, para correr tests sin depender del scraper.

- [ ] **HdU-18 — Snapshot tests por tool**
  Como dev, quiero un snapshot por cada tool con input/output esperado contra las fixtures, para detectar regresiones silenciosas. Incluye snapshot del log emitido por cada tool.

- [ ] **HdU-19 — GitHub Actions: typecheck + lint + test**
  Como dev, quiero CI en cada PR que corra `pnpm typecheck && pnpm lint && pnpm test`, para no mergear regresiones.

### Distribución

- [ ] **HdU-20 — README de uso para Claude Desktop**
...
- [ ] **HdU-21 — Release v0.1 con `cmf_norms.db`**
  Como mantenedor, quiero un GitHub Release `v0.1` con `cmf_norms.db` adjunto, SHA256 publicado en notas y tag semver del corpus, para distribuir sin que el usuario corra el scraper.
...
- [ ] **HdU-24 — Ingest de Circulares**
  Como dev, quiero extender `loader` y `normalizer` para soportar Circulares (id `circ-{numero}`), para ampliar la cobertura del corpus.

- [ ] **HdU-25 — Ingest de Oficios Circulares**
...
- [ ] **HdU-26 — Validación de hashes vs fuente CMF**
...
- [ ] **HdU-27 — Paquete npm publicable (`@gubaros/cmf-mcp`)**
...
- [ ] **HdU-28 — Auto-update de DB con verificación de SHA256**
  Como usuario, quiero que el `npx` chequee semanalmente si hay nueva versión del corpus, descargue solo si SHA256 difiere, y verifique integridad antes de reemplazar, para mantenerme actualizado sin intervención.
...
- [ ] **HdU-29 — Exponer histórico vía `get_article` con `asOfDate`**
...
- [ ] **HdU-30 — Tool `list_article_versions`**
...
- [ ] **HdU-35 — `validation_decisions.md` versionado**
